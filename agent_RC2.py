...
def vector_datastore():
    ...
    with open(log_filename, 'w') as log_file:
        ...
        vstore3 = AstraDBVectorStore(
            collection_name="land_assessment",
            embedding=embedding,
            token=os.getenv("ASTRA_DB_APPLICATION_TOKEN"),
            api_endpoint=os.getenv("ASTRA_DB_API_ENDPOINT"),
        )
        ...
        retriever_tool = create_retriever_tool(retriever, "contract_info",
                                ...
                                "Search for compensation and benefits offered by the companies to their employees.")
        ...
        retriever03 = vstore3.as_retriever(search_kwargs={"k": 3})
        retriever_tool3 = create_retriever_tool(retriever03, "land_assessment",
                        "Search for information related to legal aspects, procedures, and key documents involved in land transactions in India.")

        tools = [retriever_tool, retriever_tool2, retriever_tool3]
...
class LanguageModelProcessor:
    def __init__(self):
        ...
        with open('scratchpad_prompt_3.txt', 'r') as file:
            scratchpad_prompt = file.read().strip()
            self.scratch_pad=scratchpad_prompt
        ...
        -"Sure thing!"
        -"Ah, gotcha! That’s a great question."
        -"Alright, let’s break this down for you."
        -"Hmm, interesting! Here’s what I found."
        -"Oof, that’s a tricky one—but I’ve got you covered."
        -"Okay, here’s the lowdown!"
        When greeted, respond with greetings only.
        ...
        -contract_info: Search for compensation and benefits offered by the companies to their employees.
        ...
        -land_assessment: Search for information related to legal aspects, procedures, and key documents involved in land transactions in India.
...
# Use the following format for agent_scratchpad but only give the final answer in response to the user:
#         [Question]: the input question you must answer
#         [Thought]: you should always think about what to do
#         [Action]: the action to take, should be one of [contract_info, user_uploaded_contract]
#         [Action] Input: the input to the action
#         [Observation]: the result of the action... (this Thought/Action/Action Input/Observation can repeat not more than 3 times to reach the final answer)
#         [Thought]: I now know the final answer
#         [Final Answer]: the final answer to the original input question which is returned as response. Answer the following question concisely and directly. Do NOT include any intermediate reasoning steps, only provide the final answer.
...
#         {scratchpad_prompt}
        ...
        # self.agent_executor = AgentExecutor(agent=self.agent, tools=self.tools, memory=self.memory, verbose=True, return_intermediate_steps=False)
    ...
    def process(self, text):
        ...
        try:
            ...
            response = self.agent_executor.invoke({
                ...
                "agent_scratchpad": self.scratch_pad  # Pass scratchpad prompt
            })
        ...
        # switch between what to return during testing and running
        # for testing
